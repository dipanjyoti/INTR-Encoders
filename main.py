...
def get_args_parser():
    ...
    parser.add_argument('--noise_frac', default=0.1, type=float,
                        help='noise to new queries to load pretrained model')
    ...
    # * Encoder parameters
    parser.add_argument('--data_transform', default="vit", type=str, choices=["detr", "swin", "vit", "sam"])
    parser.add_argument('--encoder_op_dim', default=384, type=int, choices=[192, 384, 768, 786, 1024, 1280],
                        help="Number of encoder output dimension")
    parser.add_argument('--img_size', default=512, type=int, choices=[192, 224, 384, 518, 1024],
                        help="Number of encoder output dimension")
    parser.add_argument('--encoder', default= 'vit_small_patch16_224.dino', type=str, choices=['vit_small_patch16_224.dino', ])
                        # for DINO: ['vit_small_patch16_224.dino']   DINOv2: ['vit_large_patch14_dinov2.lvd142m']
                        # DeiT: ['deit_tiny_patch16_224.fb_in1k', 'deit_small_patch16_224.fb_in1k', 'deit_tiny_distilled_patch16_224.fb_in1k',]
...
def main(args):
    ...
    if args.finetune:
        if args.finetune.startswith('https'):
            decoder_checkpoint = torch.hub.load_state_dict_from_url(
                args.finetune, map_location='cpu', check_hash=True)
        else:
            decoder_checkpoint = torch.load(args.finetune, map_location='cpu')
        decoder_dict = decoder_checkpoint['model']

        model_dict=utils.load_model(args, decoder_dict, model_without_ddp)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            args.task, model, criterion, data_loader_train, optimizer, device, epoch,
            args.clip_max_norm)
